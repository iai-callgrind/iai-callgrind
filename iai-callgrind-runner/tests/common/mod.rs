...
use anyhow::Result;
use iai_callgrind_runner::runner::tool::ToolOutputPath;
...
impl Fixtures {
    ...
    pub fn get_callgrind_output_path<T>(path: T) -> ToolOutputPath
    where
        T: AsRef<Path>,
    {
        ToolOutputPath::from_existing(Fixtures::get_path_of(path)).unwrap()
    }
